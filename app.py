...
def initialize_session_state():
    ...
    # Variables de estado para los datos de descarga (se inicializan como None)
    ...
    # Esta bandera nos ayudará a saber cuándo mostrar los botones de descarga
    if 'show_downloads_after_generation' not in st.session_state:
        st.session_state.show_downloads_after_generation = False
...
def generate_pdf_bytes(data):
    try:
        ...
        for i, item in enumerate(data["conceptos"]):
            ...
            pdf.set_xy(col_widths[0] + col_widths[1], current_y)
            pdf.cell(col_widths[2], desc_height, f"{item['precio']:.2f}", 1, 0, 'C', True)
            
            item_total = item["cantidad"] * item["precio"]
            pdf.set_xy(col_widths[0] + col_widths[1] + col_widths[2], current_y)
...
if st.button("GENERAR PRESUPUESTO"):
    ...
    st.session_state.json_download_data = None
    st.session_state.download_filename_base = "" 
    st.session_state.show_downloads_after_generation = False # Reinicia la bandera
    ...
    # Si el PDF se genera con éxito, almacena los datos y la bandera para mostrar los botones
    if pdf_output:
        st.session_state.pdf_download_data = pdf_output
        st.session_state.json_download_data = json.dumps(budget_data, ensure_ascii=False, indent=4).encode('utf-8')
        st.session_state.download_filename_base = filename_base
        st.session_state.show_downloads_after_generation = True # ¡Activa la bandera!
        st.success("Presupuesto generado. Puedes descargarlo abajo.")
    else:
        st.error("No se pudo generar el presupuesto. Revisa los mensajes de error.")
    
    st.experimental_rerun() # Fuerza una re-ejecución para que se muestren los botones de descarga


# --- Mostrar botones de descarga solo si la bandera está activa ---
# Estos botones solo se renderizarán si st.session_state.show_downloads_after_generation es True
if st.session_state.get('show_downloads_after_generation', False):
